...
不再使用旧版 sh.600000 风格（v3 数据源切换至 Tushare 后的用户决策）。
...
为活代码（被 app/main.py、baostock_chanlun.py、mootdx_chanlun.py 引用），
一并迁入。
"""
...
# ---------------------------------------------------------------------------
# 股票代码标准化（Tushare 风格：XXXXXX.SH / XXXXXX.SZ / XXXXXX.BJ / NNNNN.HK）
# ---------------------------------------------------------------------------
def normalize_stock_code(code: str) -> str:
    ...
    支持的输入格式：
      ...
      - 已带 Tushare 后缀：600000.SH / 000001.SZ（原样返回，大小写规范化）
      - 港股带后缀：00700.HK（原样返回）
...
def get_market_type(stock_code: str) -> str:
    ...
    规则（基于 Tushare 代码格式 XXXXXX.SH/.SZ/.BJ/.HK）：
      - .HK 后缀或 ≤5 位纯数字 → 港股
      - 5xxxxx 或 159xxx → ETF
