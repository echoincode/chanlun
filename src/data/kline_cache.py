...
设计（尽量简单）：
  ...
  - 查询时先判断请求区间 [start, end] 是否已被缓存覆盖：
      缓存区间 [cached_min, cached_max] ⊇ [start, end] → 直接切片返回，不查远程
    否则查询缺失部分（左段 / 右段），合并去重后写回 CSV
  - 增量补齐：避免每次都全量重查，首次通常拉全量，之后区间命中

约定：
  - datetime 为 YYYY-MM-DD 字符串，可直接按字典序比较（对齐数据源返回格式）
  - 文件名中的 '.' 替换为 '_'，避免路径分隔歧义
  - 文件名带数据源前缀（{source}_{code}.csv），使不同数据源的同标的互不污染；
    当前唯一数据源为 stockdb，保留前缀以便后续接入其他源时隔离缓存
"""
...
def _cache_path(code: str, source: str = "stockdb") -> str:
    """缓存文件路径，按数据源分文件避免不同源数据互相覆盖。"""
    src = (source or "stockdb").lower()
    return os.path.join(CACHE_DIR, f"{src}_{_safe_name(code)}.csv")


def _load(code: str, source: str = "stockdb") -> pd.DataFrame | None:
...
def _save(code: str, df: pd.DataFrame, source: str = "stockdb") -> None:
...
def load_or_fetch(
    ...
    **fetcher_kwargs,
    ...
    不同数据源使用各自独立缓存文件，互不覆盖（当前唯一源为 stockdb）。
    ...
    Args:
        ...
        source: 数据源标识（当前为 stockdb），用于缓存文件隔离
        **fetcher_kwargs: 透传给 fetcher.fetch_daily_data 的关键字参数
                          （StockDB: frequency/adj）
...
def _call_fetcher(fetcher, code: str, start_date: str, end_date: str, kwargs: dict) -> "pd.DataFrame":
    ...
    兼容两种传法：
      - fetcher 是实例（如 StockDBFetcher）→ 取其 fetch_daily_data 方法
